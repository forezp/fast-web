...
import io.github.forezp.fastwebcommon.dto.ValidationResult;
    ...
    private static void checkError(ValidationResult result) {
        if (result.isHasErrors()) {
            ...
            if (result.getErrorMsg().size() > 0) {
                ...
                throw new FastwebException(ErrorCode.ERROR_ARGS, mapValueToString(result.getErrorMsg()));
            }
            throw new FastwebException(ErrorCode.ERROR_ARGS);
        }
    }
